								...
								<c:forEach var="category2" items="${categoryList2}">
									<c:if test="${score.USER_ID eq category2.USER_ID}">
										<th>문제풀이점수: </th><td> &nbsp; &nbsp;${category2.SCORE_TOTALPOINT}</td>
																...
																<tr>
								<c:forEach var="category3" items="${categoryList3}">
									<c:if test="${score.USER_ID eq category3.USER_ID}">
										<th>문제평가점수: </th><td> &nbsp; &nbsp;${category3.SCORE_TOTALPOINT}</td>
									</c:if>
								</c:forEach>		
								</tr>
							</thead>
...
<br>
    ...
    <br>
		...
		<table class="score_table">
			...
			<c:choose>
				<c:when test="${categoryInfo3 !=null }">
					<c:forEach var="list3" items="${categoryInfo3}" varStatus="scoreNum">
						<tr align="center">
							<td class="score_cell">${scoreNum.count}</td>
							<td class="score_cell">${list3.PRO_NUM}</td>	
							<td class="score_cell">${list3.PRO_NAME}</td>
							<td class="score_cell">${list3.SCORE_POINT}</td>
							<td class="score_cell"><fmt:formatDate pattern="yyyy-MM-dd" value="${list3.SCORE_DATE}"/></td>
						</tr>
					</c:forEach>
				</c:when>
			</c:choose>
...
<div style ="align: center; width: 1100px; margin:auto;"><br>
    ...
    <table>
    	...
    	<div>
    		<a href ="${contextPath}/F/F_P001/categoryScore.page?score_content=문제제작&category_id=${score.CATEGORY_ID}" >문제출제랭킹</a>
    		<a href ="${contextPath}/F/F_P001/categoryScore.page?score_content=문제집제작&category_id=${score.CATEGORY_ID}" >문제집랭킹</a>
    		<a href ="${contextPath}/F/F_P001/categoryScore.page?score_content=평가&category_id=${score.CATEGORY_ID}" >문제평가랭킹</a>
    	</div>
    	<div id = "search">
