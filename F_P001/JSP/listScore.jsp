...
<script type = "text/javascript">
	var loopSearch = true;
	function user_idSearch(scoreList){
		console.log(scoreList);
		if(loopSearch == false)
			return;
			var value = document.frmSearch.searchUser.value;
				console.log(${scoreList[0].CATEGORY_ID});
				$.ajax({
					type   : "get",
					async  : "${contextPath}/F/F_P001/searchUser.page",
					data   : {user_id:value,
								category_id:scoreList[0].CATEGORY_ID
							  },
							...
							<thead>
								...
								<c:forEach var="category3" items="${categoryList3}">
									<c:if test="${score.USER_ID eq category3.USER_ID}">
										<th>평가점수: </th><td> &nbsp; &nbsp;${category3.SCORE_TOTALPOINT}</td>
									</c:if>
								</c:forEach>	
								</tr>
							</thead>
					...
					<table>
						...
						<tr>
							<c:forEach var="category3" items="${categoryList3}">
								<c:if test="${score.USER_ID eq category3.USER_ID}">
									<th>평가점수: </th>
									<td> &nbsp;${category3.SCORE_TOTALPOINT}</td>
								</c:if>
							</c:forEach>	
						</tr>
					</table>
